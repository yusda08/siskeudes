    ...
    $(document).ready(function () {
        ...
        function load_data(status = '') {
            $('.tabel-server-side').DataTable({
                ...
                deferRender: true,
                ajax: {
                    url: "<?= site_url('surat_masuk/data_surat/load_surat_umum'); ?>",
                    type: "POST",
                    data: {status: status},
                },
            ...
            const viewSuratHtml = (row) => {
                return `<a target="_blank" href="${siteUrl(`surat_masuk/umum/view_surat?no_reg=${row.no_reg}`)}"
                class="btn btn-link btn-xs">${row.no_surat}</a>`;
            }
            const statusHtml = (row) => {
                return `<a target="_blank" href="${siteUrl(`laporan/disposisi/cetak_lembar_disposisi?no_reg=${row.no_reg}`)}"
