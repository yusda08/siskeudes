...
class BaseBuilder
{
    ...
    function on_duplicate($set = NULL)
    {
        if (!is_null($set)) {
            $this->set($set);
        }
        ...
        if ($is_multi) {
            $sql = $this->_multi_duplicate_insert($this->db->protectidentifiers($table, TRUE, NULL, FALSE), $this->QBSet);
        } else {
            $sql = $this->_duplicate_insert($this->db->protectidentifiers($table, TRUE, NULL, FALSE), $this->QBSet);
        }
